...
module Ratyrate
  ...
  def update_rate_average_dirichlet(stars, dimension = nil)
    ...
    posterior = dp.merge(stars_group){ |_key, a, e| a + e }
    sum = posterior.map{ |_, v| v }.inject{ |a, e| a + e }
    davg = posterior.map{ |i, v| i * v }.inject{ |a, e| a + e }.to_f / sum
  ...
  module ClassMethods
    ...
    def ratyrate_rateable(*dimensions)
      ...
      dimensions.each do |dimension|
        has_many "#{dimension}_rates".to_sym,
          -> { where dimension: dimension.to_s },
          dependent: :destroy,
          class_name: 'Rate',
          as: :rateable

        has_many "#{dimension}_raters".to_sym,
          through: :"#{dimension}_rates",
          source: :rater

        has_one "#{dimension}_average".to_sym,
          -> { where dimension: dimension.to_s },
          as: :cacheable,
          class_name: 'RatingCache',
          dependent: :destroy
